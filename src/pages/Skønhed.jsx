...
const Skonhed = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const roomData = await fetchRoomInfo("Skønhed");
        const artworksData = await fetchArtworks("Skønhed");
        const sortedArtworks = artworksData.sort((a, b) => a.order - b.order);

        setRoomInfo(roomData || { title: "Skønhed", description: "" });
